...
class Ratios:
    ...
    def update_type(self, power, cap, bar, type_):
        ...
        setattr(self, f'{type_}_current_ratio_cap',
                int(getattr(self, f'stats_{type_}_base_cap') / getattr(self, f'stats_{type_}_base_power')))
        setattr(self, f'{type_}_current_ratio_bar',
                round(getattr(self, f'stats_{type_}_base_bar') / getattr(self, f'stats_{type_}_base_power'), 2))
        ...
        current_goal_ratio_cap = (
                getattr(self, f'stats_{type_}_base_cap') /
                getattr(self, f'{type_}_edit_cap') /
                getattr(self, f'stats_{type_}_base_power') *
                getattr(self, f'{type_}_edit_power')
        )
        current_goal_ratio_bar = (
                getattr(self, f'stats_{type_}_base_bar') /
                getattr(self, f'{type_}_edit_bar') /
                getattr(self, f'stats_{type_}_base_power') *
                getattr(self, f'{type_}_edit_power')
        )
        max_of_current_goal_ratio = max(current_goal_ratio_power, current_goal_ratio_cap, current_goal_ratio_bar)
        #   BASE GOAL
        setattr(self, f'{type_}_base_goal_power',
                ceiling_precise(getattr(self, f'stats_{type_}_base_power') * max_of_current_goal_ratio, 0.01))
        setattr(self, f'{type_}_base_goal_cap', ceiling_precise(
            getattr(self, f'{type_}_base_goal_power') / getattr(self, f'{type_}_edit_power') * getattr(self,
                                                                                                       f'{type_}_edit_cap'),
            250))
        setattr(self, f'{type_}_base_goal_bar', math.ceil(
            getattr(self, f'{type_}_base_goal_power') / getattr(self, f'{type_}_edit_power') * getattr(self,
                                                                                                       f'{type_}_edit_bar')))
        # AMOUNT LEFT TO BUY
        setattr(self, f'{type_}_amount_left_to_buy_power',
                getattr(self, f'{type_}_base_goal_power') - getattr(self, f'stats_{type_}_base_power'))
        setattr(self, f'{type_}_amount_left_to_buy_cap',
                getattr(self, f'{type_}_base_goal_cap') - getattr(self, f'stats_{type_}_base_cap'))
        setattr(self, f'{type_}_amount_left_to_buy_bar',
                getattr(self, f'{type_}_base_goal_bar') - getattr(self, f'stats_{type_}_base_bar'))
        # EXP COST
        ...
        #   TOTAL EXP COST
        setattr(self, f'{type_}_amount_left_to_buy_sum',
                getattr(self, f'{type_}_exp_cost_bar') + getattr(self, f'{type_}_exp_cost_power') + getattr(self,
                                                                                                            f'{type_}_exp_cost_cap'))
    ...
    def update_me(self, em_energy=6.0, em_magic=2.0):
        ...
        self.em_goal_em = em_energy / em_magic
        #   OPTIMAL ENERGY FOR MAGIC AT RATIO
        self.me_optimal_for_current_power = round(
            ceiling_precise(self.stats_magic_base_power / min(1, self.em_goal_em / self.em_cr_power), 0.1), 2)
        self.me_optimal_for_current_cap = round(
            ceiling_precise(self.stats_magic_base_cap / min(1, self.em_goal_em / self.em_cr_cap), 10000), 2)
        self.me_optimal_for_current_bar = round(
            ceiling_precise(self.stats_magic_base_bar / min(1, self.em_goal_em / self.em_cr_bar), 1), 2)
        #   AMOUNT LEFT TO BUY
    ...
    def update_er3(self, energy=100000, r3=1):
        ...
        self.er3_optimal_for_current_power = round(
            ceiling_precise(self.stats_r3_base_power / max(1, self.er3_goal_er3 / self.er3_cr_power), 0.1), 2)
